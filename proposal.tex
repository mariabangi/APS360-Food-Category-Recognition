...
Food recognition has become increasingly important as applications such as health and diet monitoring become more mainstream, restaurant and delivery services become automated, and accessibility tools are developed for impaired individuals. Accurate classification of food images enables calorie estimations, nutritional analysis, and improved user interaction. The goal of this project is to develop a deep learning model that will classify food images into predetermined food categories such as 'pizza' and 'pasta'.   

This task is challenging due to large visual variation within food categories and contrarily, visual similaries between different categories. Variations in lighting, presentation, aesthetics and viewpoints complicate classification. Convolutional neural networks (CNN's) are suited for this due to strong performance on image classification by its ability to automatically learn significant visual features from raw pixel data given. This project will train and evaluate a CNN-based approach which will be suitable and an effective method for food category recognition. Future integrattion of the model will be nutritional applications that will automatically identify food categories and calorie intake from user-uploaded images.
...
A CNN implemented in PyTorch using transfer learning will be used. A pretrained backbone may be incorperated like preexisting models, such as ResNet or EfficientNet, if feasible within the course scope. From existing models, the final classification layer will be replaced with a new fully connected layer that will output probabilities for the top food categories as the output. A softmax function will be applied to produce class probabilities, and as a result, training will use cross-entropy loss.
