...
The chosen data set will be split into both training and validation sets to ensure class balance. Pixel values will be normalized using the standard ImageNet statistics. Data augmentation techniques such as cropping, flipping and color adjustments will be applied to improve the lighting and interview variations. Any corrupted images that are unreadable will be removed during pre-processing so the image is not inputted, and data quality is improved through error prevention.
